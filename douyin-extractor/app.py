...
def get_video_url():
    ...
    try:
        ...
        detail_url = f"https://www.douyin.com/video/{video_id}?modeFrom="
        ...
        if not aweme_detail:
            print("未找到aweme_detail，尝试从aweme_list中获取...")
            if 'aweme_list' in json_data and isinstance(json_data['aweme_list'], list) and len(
                    json_data['aweme_list']) > 0:
                aweme_detail = json_data['aweme_list'][0]
            else:
                raise Exception("无法找到视频详情数据 (aweme_detail 或 aweme_list)。")

        video_info = aweme_detail.get('video', {})
        if not video_info:
            raise Exception("视频信息不存在 (video)。")
